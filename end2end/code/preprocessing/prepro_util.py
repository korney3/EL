...
class VocabularyCounter(object):
    ...
    def __init__(self, lowercase_emb=False):
        ...
        config.base_folder+"data/basic_data/wordEmbeddings/Word2Vec/word2vec", binary=False)
...
def build_word_char_maps():
    ...
    model = gensim.models.KeyedVectors.load_word2vec_format(
        config.base_folder+"data/basic_data/wordEmbeddings/Word2Vec/word2vec")#GoogleNews-vectors-negative300.bin", binary=True)
        ...
        with open(filepath) as fin:
            ...
            for line in fin:
                ...
                if line in self.chunk_ending:
                    ...
                    line=line.lower()
                    line=line.strip()
                    if len(line)>2 and 'Name' not in morph.parse(line)[0].tag and 'Surn' not in morph.parse(line)[0].tag:
                        line = morph.parse(line)[0].normal_form
                    self.chunk_words.append(line)
